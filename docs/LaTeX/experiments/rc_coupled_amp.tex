...
\chapter{RC-Coupled Amplifier using BJT in CE Configuration}
\label{chap:expt2}
...
\begin{enumerate}
	...
	\item \textbf{Mid-Band Region:} In this region (typically \SI{100}{\hertz} to \SI{100}{\kilo\hertz}), t both coupling capacitors behave as AC short circuits and the emitter bypass capacitor provides a low-impedance AC path, so the emitter is at AC ground. Concurrently, the internal parasitic capacitances of the BJT act as open circuits. Consequently, the voltage gain remains constant and reaches its maximum value ($A_{v(mid)}$).
...
\begin{enumerate}
	...
	\item Adjust the signal generator amplitude so that the input voltage $V_{in}$ is small (e.g., $20\text{ mV}_{p-p}$), ensuring that the output waveform $V_{out}$ remains a clean, unclipped sine wave.
...
\begin{itemize}
	...
	\item Ensure correct transistor pin identification (Emitter, Base, Collector) before insertion; an incorrectly connected transistor may be damaged or bias incorrectly.
...
\begin{table}[h!]
	...
	\begin{tabular}{llll}
		\toprule
		\textbf{Parameter} & \textbf{Theoretical Value} & \textbf{Measured Value} & \textbf{\% Deviation} \\
		\midrule
		Base Voltage ($V_B$) & \SI{1.9}{\volt} & & \\
		Emitter Voltage ($V_E$) & \SI{1.2}{\volt} & & \\
		Collector Voltage ($V_C$) & \SI{7.2}{\volt} & & \\
		Collector-Emitter Voltage ($V_{CEQ}$) & \SI{6.0}{\volt} & & \\
		Collector Current ($I_{CQ}$) & \SI{2.0}{\milli\ampere} & & \\
		\bottomrule
	\end{tabular}
