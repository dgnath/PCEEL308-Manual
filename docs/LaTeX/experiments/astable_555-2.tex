...
\section{Aim}
To design, construct, and evaluate an Astable Multivibrator circuit using IC 555 for a **given target duty cycle of $60\%$** and target frequency of $1\text{ kHz}$. The specific objectives are:
\begin{enumerate}
	\item To derive $R_A$ and $R_B$ component values required for a specified duty cycle $D$ and oscillation frequency $f_0$.
	...
	\item To verify upper ($V_{TH}$) and lower ($V_{TL}$) trigger threshold levels relative to $V_{CC}$.
\end{enumerate}
...
\begin{enumerate}
	...
	\item Derive the relationship between $R_A$ and $R_B$ required to produce a duty cycle $D = 60\%$.
	\item Show that the total period $T$ for a $60\%$ duty cycle circuit reduces to $T \approx 1.7325 \cdot R_B C$.
	\item Calculate the theoretical values of $R_A$ and $R_B$ for $f_0 = 1\text{ kHz}$, $D = 60\%$, and $C = \SI{0.1}{\micro\farad}$. Select nearest standard 1\% or 5\% resistor values and recalculate expected $f_0$ and $D$.
\end{enumerate}
...
\begin{table}[h!]
	...
	\begin{tabular}{llll}
		...
		2 & Resistors & \SI{2.7}{\kilo\ohm}, \SI{5.6}{\kilo\ohm} (Standard E12) & 1 No. each \\
...
\section{Theory}
An astable multivibrator using IC 555 is a free-running relaxation oscillator. Timing is controlled by charging capacitor $C$ through $(R_A + R_B)$ and discharging it through $R_B$ via the internal discharge transistor (Pin 7).

\subsection{Timing Equations}
\begin{itemize}
	\item \textbf{High Output Duration ($t_1$):} Capacitor charges from $\frac{1}{3} V_{CC}$ to $\frac{2}{3} V_{CC}$:
	\begin{equation}
...
\subsection{Duty Cycle Formulation}
The duty cycle $D$ is defined as:
\begin{equation}
	D = \frac{t_1}{T} = \frac{R_A + R_B}{R_A + 2 R_B}
\end{equation}
For a specified duty cycle $D$, rearranging Equation (5.5) yields the direct resistor ratio required:
\begin{equation}
	R_A = \left( \frac{2D - 1}{1 - D} \right) R_B \quad \text{or} \quad R_B = \left( \frac{1 - D}{2D - 1} \right) R_A
\end{equation}
...
\begin{enumerate}
	...
	\item Recalculate theoretical performance with standard components:
	\begin{equation}
	...
	\end{equation}
	\begin{equation}
		T_{\text{theoretical}} = 0.575\text{ ms} + 0.388\text{ ms} = 0.963\text{ ms}
	\end{equation}
	\begin{equation}
		f_{0,\text{theoretical}} = \frac{1}{0.963 \times 10^{-3}\text{ s}} \approx 1038.4\text{ Hz} \quad (1.04\text{ kHz})
	\end{equation}
	\begin{equation}
		D_{\text{theoretical}} = \frac{2.7 + 5.6}{2.7 + 2(5.6)} \times 100\% = \frac{8.3}{13.9} \times 100\% = 59.71\% \approx 60\%
	\end{equation}
\end{enumerate}
