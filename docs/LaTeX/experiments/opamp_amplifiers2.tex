...
\begin{figure}[h!]
	...
	\begin{circuitikz}[scale=0.9, line width=0.8pt, transform shape]
		...
		\draw (opamp.-) -- (-1.5, 0.5) to[R, l_=$R_1\ (\SI{10}{\kilo\Omega})$] (-4, 0.5) to[sV, l_=$V_1$] (-4, -1) node[ground] {};
		...
		\draw (-1.5, -0.5) to[R, l=$R_4\ (\SI{10}{\kilo\Omega})$] (-1.5, -2.2) node[ground] {};
		
		% Output
		\draw (opamp.out) -- (2.5, 0) node[right] {$V_{out}$};
		
		% Vertical Power Lines
		\draw (opamp.up) -- ++(0, 0.5) node[above] {$+V_{CC} = +15\text{ V}$};
		\draw (opamp.down) -- ++(0, -0.5) node[below] {$-V_{EE} = -15\text{ V}$};
...
\begin{enumerate}
	...
	\item \textbf{DC Analysis:} Apply DC voltages from auxiliary supplies to $V_1$ and $V_2$ as listed in Table \ref{tab:dc_adder}. Measure $V_{out}$ with a digital multimeter and compare with $-(V_1 + V_2)$.
	\item \textbf{AC Analysis:} Apply sinusoidal inputs at \SI{1}{\kilo\hertz}: $V_1 = 1.0\text{ V}_{p-p}$ and $V_2 = 1.5\text{ V}_{p-p}$ from synchronized generator outputs. Observe $V_{out}$ on the DSO and confirm the sum and $180^\circ$ phase relation.
\end{enumerate}

\subsection{Subtractor Circuit Setup}
\begin{enumerate}
	\item Reconfigure the circuit according to Figure \ref{fig:subtractor_circuit}.
	\item \textbf{DC Test:} Apply DC voltages to $V_1$ and $V_2$ as listed in Table \ref{tab:dc_subtractor}. Measure $V_{out}$ using a multimeter and verify $V_{out} = V_2 - V_1$.
	\item \textbf{AC Test:} Apply two sinusoidal signals at \SI{1}{\kilo\hertz} with unequal amplitudes ($V_1 = 1.0\text{ V}_{p-p}, V_2 = 2.5\text{ V}_{p-p}$) in phase\footnote{If two sources with in-phase signals are not available, you may derive the second source using a potential divider network.}. Measure $V_{out}$ on the DSO and verify that $V_{out} = 1.5\text{ V}_{p-p}$.
\end{enumerate}
...
\begin{figure}[h!]
	...
	\begin{tikzpicture}
		\begin{axis}[
			...
			title={\textbf{Expected Inverting Adder Waveforms ($V_1 = 1.0\text{ V}$, $V_2 = 1.5\text{ V}$)}},
			legend pos=outer north east
			]
			% Input V1
			\addplot[thick, blue, domain=0:2, samples=100] {1.0*sin(deg(2*pi*x))};
			\addlegendentry{$V_1\ (1.0\text{ V}_{p-p})$}
			
			% Input V2
			\addplot[thick, green!60!black, dashdotted, domain=0:2, samples=100] {1.5*sin(deg(2*pi*x))};
			\addlegendentry{$V_2\ (1.5\text{ V}_{p-p})$}
			
			% Summed Output Vout = -(V1 + V2)
			\addplot[ultra thick, red, domain=0:2, samples=100] {-2.5*sin(deg(2*pi*x))};
			\addlegendentry{$V_{out}\ (-2.5\text{ V}_{p-p})$}
		\end{axis}
	\end{tikzpicture}
	\caption{Time-Domain Waveforms for Inverting Summing Amplifier}
