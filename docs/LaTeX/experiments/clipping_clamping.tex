...
Figure~\ref{fig:shunt_pos_clipper} shows a shunt clipper with the diode across the output. Whenever $v_{in} > V_F (\approx 0.7\ \text{V})$, the diode conducts and clamps the output near $V_F$; for $v_{in} < V_F$, the diode is OFF and $v_{out} = v_{in}\frac{R_L}{R_L+R}$. If $R_L \gg R$, $v_{out}\approx v_{in}$, during the negative cycle . The waveforms expected are as given in Fig.~\ref{fig:wavePosClipper}. 
...
Figure~\ref{fig:shunt_neg_clipper} shows a shunt negative clipper with the diode across the output, placed in the opposite polarity compared to the positive clipper. Whenever $v_{in} < -V_F$, the diode conducts and clamps the output near $-V_F$; for $v_{in} > -V_F$, the diode is OFF and $v_{out} = v_{in}\dfrac{R_L}{R_L+R}$. If $R_L \gg R$, $v_{out}\approx v_{in}$, during the positive cycle. The waveforms expected are as given in Fig.~\ref{fig:waveNegClipper}. 
...
The transfer characteristics graphs are given here:
\begin{figure}[htbp]
	\centering
	% --- POSITIVE CLIPPER ---
	...
	\caption{Transfer Characteristics of Unbiased Clippers}
\end{figure}
Transfer Characteristics of Biased clippers.
\begin{figure}[htbp]
	...
	\begin{tikzpicture}
		\begin{axis}[
			...
			% Clipping behavior
			\addplot[ultra thick, blue, domain=-5:5, samples=200] {min(x, -2)};
			% Reference Guide Lines
			\draw[dashed, gray] (axis cs:-2,0) -- (axis cs:-2,-2) node[below left, black] {$-V_R$};
			\draw[dashed, gray] (axis cs:0,-2) -- (axis cs:-2,-2);
		\end{axis}
	\end{tikzpicture}
	\caption{Transfer Characteristics of Biased Positive Clippers}
\end{figure}
...
\begin{figure}[htbp]
	...
	\begin{tikzpicture}
		\begin{axis}[
			...
			% Clipping behavior
			\addplot[ultra thick, red, domain=-5:5, samples=200] {max(x, 2)};
			% Reference Guide Lines
			\draw[dashed, gray] (axis cs:2,0) -- (axis cs:2,2) node[above left, black] {$V_R$};
			\draw[dashed, gray] (axis cs:0,2) -- (axis cs:2,2);
		\end{axis}
	\end{tikzpicture}
	\hfil
...
\begin{figure}[htbp]
	...
	\begin{subfigure}{0.52\textwidth}
		...
		\begin{subfigure}{0.52\textwidth}
			...
			\label{fig:transferCharaPosClip}
		\end{subfigure}
	\end{subfigure}
	\hfill
	%	% ---------- RIGHT: stacked plots ----------
	...
	\end{subfigure}	
	\caption{Diode clipper circuit with input/output waveforms and transfer characteristics}
...
\begin{figure}[htbp]
	...
	\begin{subfigure}{0.42\textwidth}
		\begin{tikzpicture}
			...
			\end{axis}
		\end{tikzpicture}
		\caption{Input and output waveforms for the biased positive clipper of Figure~\ref{fig:biasedSourceClipper}: the output is flattened above $V_F$.}
	...
	\begin{subfigure}{0.5\textwidth}
		...
		\begin{tikzpicture}
			\begin{axis}[
				...
				\addplot[ultra thick, blue, domain=-5:5, samples=200] {min(x, 2)};
				% Reference Guide Lines
				\draw[dashed, gray] (axis cs:2,0) -- (axis cs:2,2) node[above right, black] {$V_R$};
				\draw[dashed, gray] (axis cs:0,2) -- (axis cs:2,2);
			\end{axis}
		\end{tikzpicture}
		\caption{Transfer characteristics of biased positive clipper with clipping level at ($V_R+V_F=+2 V$}
	\end{subfigure}
	\
\end{figure}
\begin{figure}[htbp]
...
\begin{figure}[hbtp]
	...
	\begin{subfigure}{0.6\textwidth}
		\centering
		\begin{circuitikz}[american, scale=1.2, line width=0.8pt]
			\draw
			(0,0) to[sinusoidal voltage source, l=$v_{in}(t)$] (0,3)
			(0,3) to[R, l=$R$, *-*] (2.8,3)
			(2.8,3) to[short] (5,3)
