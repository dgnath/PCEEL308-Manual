...
\begin{enumerate}
	...
	\end{equation}
	\begin{equation}
		R_D = \frac{V_{RD}}{I_D} = \frac{5.4\text{ V}}{2\text{ mA}} = \SI{2.7}{\kilo\Omega} \quad (\text{Select standard value: } \SI{2.7}{\kilo\Omega}).
	\end{equation}
	
	\item \textbf{Design of Biasing Network ($R_1$ and $R_2$):}
	
	The transfer characteristics of 2N7000 gives:
	\[
	V_{GS}\approx 2.3-2.7\ V
	\]
	for $I_D= 2\ mA$.
	
	Take $V_{GS}\approx 2.5\ V$.
	
	Required gate voltage 
	\[
	V_G=V_S+V_{GS} = 1.2 \ V+2.5\ V = 3.5 \ V
	\]
	
	Choose divider current of approximately $20\mu A$. \footnote{For higher input impedance, a lower divider current may be assumed, which results in higher divider resistance values.}
	
	Then, 
	\[
	R_1+R_2 = \dfrac{12\ \text{V}}{20\ \mu A} \approx 600\ k\Omega
	\]
	\[
	\dfrac{R_2}{R_1+R_2}= \dfrac{3.5}{12} = 0.292
	\]
	$R_2=0.292\times600\  k = 175\ k$; Choose a standard value of $R_2=180\ k\Omega$.
	
	Then $R_1=600\ k-180\ k= 420\ k$. Choose a standard value of $R_1=470\ k\Omega$. 
...
\section{Procedure}
Set up the circuit as shown in the Figure \ref{fig:mosfet_circuit}. Before giving the input ac signal, check the bias voltages. Apply an input sinusoidal signal of 40 mV (peak-to-peak) at 1000 Hz. Observe the output on the DSO. Vary the frequency of the input signal over a range of values (from 50Hz to a few MHz) to obtain the frequency response which is a graph between log f (x-axis) and gain in dB (y-axis) (See Table 	\ref{tab:ac_obs_mos}).
...
\begin{table}[h!]
	...
	\begin{tabularx}{\textwidth}{lXll}
		\toprule
		\textbf{Parameter} & \textbf{Theoretical Value} & \textbf{Practical Value (Measured)} & \\
		\midrule
		Gate Voltage ($V_G$) &  &  \\
		Source Voltage ($V_S$) &  &  \\
		Drain Voltage ($V_D$) &  &  \\
